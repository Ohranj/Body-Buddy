...
<template>
    <div class="h-[75px] flex items-center justify-end">
        <div class="relative" id="dropdown">
            <button @click.stop="show = !show" class="rounded-md cursor-pointer p-2 text-sm min-w-[95px] flex items-center gap-x-1"><span v-text="user.forename + ' ' + user.surname"></span>
                    ...
                    <div>
                        <div class="hover:bg-slate-300 cursor-pointer">
                            <Form method="POST" action="/logout">
                                ...
                                <button class="text-xs cursor-pointer w-full text-left p-1.5 flex items-center gap-x-2" type="submit"><span><Logout class="w-4 h-4" stroke="#000000" fill="none" /></span>Logout</button>
                            </Form>
                        </div>
                    </div>
                </div>
